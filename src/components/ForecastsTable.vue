...
<table>
  ...
  <tbody>
    <tr v-for="forecast in preparedForecasts">
      ...
      <td class="action-buttons">
        ...
        <div class="copy-button action-button" @click="copy(forecast)">
  ...
  <ModalWindow v-if="showCopyModal" @close="showCopyModal = false" :width="'400px'">
    <template v-slot:header>
      <h3>Копирование прогноза {{ chosenForecastName }}</h3>
    </template>
    <template v-slot:body>
      <CopyForecast :forecast-id="chosenForecast"></CopyForecast>
    </template>
  </ModalWindow>
  </transition>
</template>
...
export default {
  ...
  methods: {
    copy(chosenForecast) {
      this.chosenForecast = chosenForecast.id;
      this.showCopyModal = true;
      this.chosenForecastName = chosenForecast.name;
    },
    deleteForecast(id) {
